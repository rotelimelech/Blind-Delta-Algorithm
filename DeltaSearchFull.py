...
def fit_Convergence_Rate(pn, qn, depth):
    """
    ...
    difference = [log(abs(mpfr(pn[depth])/mpfr(qn[depth]) - mpfr(pn[n-1])/mpfr(qn[n-1])))  for n in depths]
...
def delta2(L, p, q, gcd, rational_marker):
    """
...
def calc_individual(coefficients, coefficients_lengths, depth, precision, not_calculated_marker, rational_marker, LIMIT_CONSTANT):
    """
    ...
    PCFdata["c"], PCFdata["d"], PCFdata["c_SDS"], PCFdata["d_SDS"] = format_with(fit_Normalized_Qn(recurrence_relations_data[1], recurrence_relations_data[2], depth), 10, "f")
    ...
    #Prepare and calculate the naive and normalized deltas using delta2.
    p = mpz(recurrence_relations_data[0][int(depth / 2)])
    q = mpz(recurrence_relations_data[1][int(depth / 2)])
    gcd = mpz(recurrence_relations_data[2][int(depth / 2)])

    PCFdata["Naive_Delta"], PCFdata["FR_Delta"] = format_with(delta2(limit_evaluation, p, q, gcd, rational_marker), 10, "f")
    ...
    #Fit the convergence rate of the PCF
    convergence_params, convergence_covariance = fit_Convergence_Rate(recurrence_relations_data[0], recurrence_relations_data[1], depth)
...
def search(depth, coefficients_lengths, co_min, co_max, precision, not_calculated_marker, rational_marker, LIMIT_CONSTANT, n_cores):
    ...
    filename = f"DeltaSearch{coefficients_lengths}_{co_min}_{co_max}.csv"
    ...
    all_combinations = [
        (list(combo), coefficients_lengths, int(depth), precision, not_calculated_marker, rational_marker, LIMIT_CONSTANT) for combo in combinations
    ]
...
def main():
    search(2000, [3, 3], -1, 1, 100000, -1010, -2020, 1000000000,3)
